...
### Стъпка 5 — Активиране на Лиценза (Първо Стартиране)

Вашият пакет GeneForge е защитен от **лиценз, обвързан с машината**. Преди да стартирате съветника, уверете се, че файлът `license.key` е наличен в директорията на съветника.

**Ако вече сте получили `license.key`:**
```bash
cp license.key ~/my-clone/wizard/
```

**Ако трябва да заявите лиценз:**
1. Изпълнете колектора UUID на вашия DGX Spark (не изисква sudo):
   ```bash
   bash collect-uuid.sh
   ```
   > Чете `/etc/machine-id` по подразбиране. Ако вашата система използва DMI UUID, скриптът го открива автоматично.
2. Изпратете изхода по имейл на **info@geneforge.eu** с тема:
   `[Име на Фирма] – System UUID за активиране на лиценз`
3. GeneForge ще ви отговори с вашия `license.key` в рамките на 24 часа.

**Ако преинсталирате операционната система:**
System UUID може да се промени. Свържете се с поддръжката на GeneForge за **безплатно нулиране на лиценза** (едно годишно включено в повечето договори).

---

### Стъпка 6 — Стартиране на съветника за onboardинг (Onboarding Wizard)
