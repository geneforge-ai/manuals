...
- **Quantizzazione di default**: `Q4_K_M` (da maggio 2026) — garantisce massima stabilità su DGX Spark. FP4 verrà riabilitato dopo il fix ufficiale NVIDIA per GB10.
...
Se vuoi andare avanti manualmente, esegui `build` dopo aver personalizzato il blueprint. In alternativa, usa sempre `test-run`.
...
| `ImportError` | Venv non attivato | `source ~/quigley_training/venv/bin/activate` |
| `add-doc` chiede `--summary` | Argomento obbligatorio mancante | Aggiungi `--summary "..."` |
| Upload file non funziona su Firefox | Firefox Snap blocca l'accesso al filesystem | Usa Chrome/Chromium o vedi [firefox-snap-upload-fix](https://github.com/geneforge-ai/firefox-snap-upload-fix) |
...
- Il pacchetto `.geneclone` è un file `.zip` rinominato: portabile, non legato a hardware specifico.
